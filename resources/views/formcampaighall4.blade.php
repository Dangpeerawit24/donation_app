...
<script>
    ...
    window.onload = function() {
        fetch('/fetch_formcampaighall2_details')
    ...
    function createInputFields(index) {
        ...
        const nameTextarea = `
        <textarea name="newName[]" id="newDonorName${index}" class="mt-2 mb-1" style="width: 100%;  height: 150px;" placeholder="กรอก รายนามผู้ส่ง  ชุดที่ ${index + 1}"></textarea>`;

        // สร้าง textarea สำหรับคำอธิษฐาน
        const wishTextarea = `
        <textarea name="newWish[]" id="newDonorWish${index}" class="mt-2 mb-1" style="width: 100%;  height: 150px;" placeholder="กรอก รายนามผู้รับ ชุดที่ ${index + 1}"></textarea>`;
    ...
    function checkNewEntry(select, index) {
        ...
        const newInput2 = document.getElementById(`newDonorWish${index}`);
        if (select.value === "new") {
            newInput.style.display = 'block';
            newInput.required = true;
            newInput.value = '';
            newInput2.style.display = 'block';
    ...
    function submitForm() {
        ...
        const donorInputs2 = document.querySelectorAll('[id^="donorWish"], [id^="newDonorWish"]');
        ...
        const allInputsFilled2 = Array.from(donorInputs2).every(input => {
            if (input.style.display !== 'none') {
                return input.value.trim() !== "";
            }
            return true;
